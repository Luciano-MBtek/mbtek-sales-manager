...
type StepTwoCompleteSystemProps = {
  dealId: string;
  contactId: string;
  dealOwnerId: string;
  products: any[];
...
const StepTwoCompleteSystem = ({
  ...
  console.log("Update main product?", updateMainProduct);
  ...
  const handleFormSubmit = createFormSubmitHandler(
    "/api/generate-complete-quote",
    {
      ...
      setRedirectOptions,
      toast,
      resetLocalStorage: () => {}, // No need to reset local storage in this case
    }
  );

  const handleFormUpdate = createFormSubmitHandler(
    "/api/update-complete-quote",
    {
      setIsSubmitting,
      setHasError,
      setIsComplete,
...
export default StepTwoCompleteSystem;
