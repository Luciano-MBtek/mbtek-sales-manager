...
export default async function DealsLayout({ children, params }: Props) {
  ...
  const steps = [
    {
      title: "Contact data",
      route: "step-one",
      link: `/forms/quick-quote/${id}/quote/${dealId}/step-one`,
    },
    {
      title: "Add products",
      route: "step-two",
      link: `/forms/quick-quote/${id}/quote/${dealId}/step-two`,
    },
    {
      title: "Review",
      route: "step-three",
      link: `/forms/quick-quote/${id}/quote/${dealId}/review`,
    },
  ];
  ...
  return (
    <div className="w-full px-2 lg:px-0 mt-[--header-height]">
      <PageHeader
        title={`Quick Quote - ${hasQuote ? "Update" : "Creation"}`}
