...
export const stepTwoQuickQuoteAction = async (
  ...
  if (products && Array.isArray(products)) {
    try {
      ...
      if (allProductsHaveFreeShipping) {
        ...
        if (validProducts) {
          const { firstname, lastname, address, city, state, zip, country } = lead;
          const now = new Date();
          ...
          const commodities = productProperties
            ...
            .map((product: any) => {
              ...
              return {
                ...
                nmfc: product.nmfc,
                quantity: originalProduct ? Number(originalProduct.quantity) : 0,
  ...
  redirect(
    `/forms/quick-quote/${contactId}/quote/${dealId}/review?${searchParams.toString()}`
  );
};
