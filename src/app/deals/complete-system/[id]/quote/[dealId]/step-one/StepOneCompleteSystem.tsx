...
import { stepOneCompleteSystemAction } from "./actions";
...
type StepOneCompleteSystemProps = {
  dealId: string;
  contactId: string;
...
export default function StepOneCompleteSystem({
  dealId,
  contactId,
  initialProducts = [],
  purchaseOptions = [],
  ...
  mainProductSku = "",
  splitPayment = "",
}: StepOneCompleteSystemProps) {
  const [serverErrors, formAction] = useActionState(
    stepOneCompleteSystemAction,
    initialState
  );
  ...
  const handleRemoveProduct = (productId: string) => {
    setSelectedProducts((prevProducts) => {
      ...
      // If we removed the main product and have other products, select a new main
      if (isRemovingMainProduct && filteredProducts.length > 0) {
        return filteredProducts.map((product, index) => ({
          ...product,
          isMain: index === 0, // Make the first remaining product the main one
        }));
      }
  ...
  // Custom form action to include the additional fields
